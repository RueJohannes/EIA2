namespace A10 {
	...
	function init(): void {
		...
		for (let i: number = 0; i < 37; i++) {
			let x: number = Math.random() * canvas.width;
			let y: number = Math.random() * canvas.height + 530;
			maleKies(x, y);
		}
	}

	//Fisch
	function maleFisch(_x: number, _y: number): void {
	...
	// Luftblasen
	function maleLuftblase1(_x: number, _y: number): void {
	...
	function maleLuftblase2(_x: number, _y: number): void {
	...
	function maleKies(_x: number, _y: number): void {
